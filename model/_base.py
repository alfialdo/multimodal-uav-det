...
class YOLOHead(pl.LightningModule)  :
    def __init__(self, x_channels:List[int], anchors, loss_balancing):
        super().__init__()
    ...
    def compute_metrics(self, outs:List[DetectionResults], batch:BatchData, return_ap=False):
        ...
        for i in range(batch_size):
            ...
            for head_idx in range(len(outs)):
                ...
                t_obj = targets[..., 0:1] # (N, 1)


                # Decode predictions to head grid space and build target
                p_bbox_decoded = self.__pred_bbox_decoding(p_bbox, self.anchors[head_idx], bbox_loss_fn='mse')
                t_bbox = self.__build_target_bbox(t_bbox, self.anchors[head_idx], bbox_loss_fn='mse')

                # Calculate losses
                bbox_losses += self.bbox_w * bbox_loss(p_bbox_decoded[target_cell], t_bbox[target_cell], bbox_loss_fn='mse')
                obj_losses += self.objectness_w * objectness_loss(p_obj[target_cell], t_obj[target_cell], self.obj_scales_w[head_idx])
